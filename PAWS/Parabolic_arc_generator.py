...
def parabolic_points(step_length, step_height, num_points):
    
     # Constants
    a = -step_height / (step_length ** 2)
    # X values for the curve
    x = np.linspace(-step_length, step_length, num_points)
    # Calculate the corresponding Z values
    z = a * x**2 + step_height - 0.25
    return x, z
...
x_arc, z_arc = parabolic_points(step_length, step_height, num_points)
x_stance = x_arc[::-1]  
...
data = data
np.savetxt('quadruped_parabolic_arc_coordinates.csv', data, delimiter=',', header='X,Z', comments='')
